$(document).ready(function() {
   $(top).on("radiant_open_character_sheet", function(_, e) {
      if (App.stonehearth.citizenCharacterSheetView) {
         App.stonehearth.citizenCharacterSheetView.destroy();
      } else {
         // only open the sheet if the player owns the entity
         App.stonehearth.citizenCharacterSheetView = App.gameView.addView(App.StonehearthCitizenCharacterSheetView, {
            uri: e.entity
         });
      }
   });
            ...
            if (buffsByAttribute[attributeName] != null) {
               ...
               for (var i = 0; i < buffsByAttribute[attributeName].length; i++) {
                  buffString = buffString + '<span class="buffTooltipText">'
                                                      + '<span class="dataSpan '
                                                      + buffsByAttribute[attributeName][i].axis
                                                      + 'DataSpan">'
                                                      + i18n.t(buffsByAttribute[attributeName][i].shortDescription)
                                                      + '</span>'
                                                      + '<img class="buffTooltipImg" src="'
                                                      + buffsByAttribute[attributeName][i].icon + '"> '
                                                      + i18n.t(buffsByAttribute[attributeName][i].display_name)
                                                      + '</span></br>';
               }
         ...
         radiant.each(allBuffs, function(k ,v) {
            ...
            if (allBuffs[k].invisible_to_player == undefined || !allBuffs[k].invisible_to_player) {
               ...
               for (var mod in modifiers) {
                  var new_buff_data = {}
                  new_buff_data.display_name = allBuffs[k].display_name;
                  new_buff_data.axis = allBuffs[k].axis;
